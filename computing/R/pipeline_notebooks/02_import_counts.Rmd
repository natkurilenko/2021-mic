...
```{r}
cntfiles <- list.files(path = cntdir, pattern = "*_ReadsPerGene.out.tab") # Just like `ls`
```
...
### Extract first and second columns of first count file
...
### Extract and rename first and second columns of first count file
...
### Extract and rename first and second columns of first and second count files and merge by gene name
...
`foreach` works like a for loop, but it has a nice `.combine` option that allows us to tell it how to combine the output at each iteration.

The following iterates over `cntfiles`. For each file in `cntfiles`, it sets the full pathname of the count file, reads it in using `read_tsv`, renames the columns, and then uses the `.combine` function to combine the output using a full-join with the result from the previous iteration, growing the data frame by one column each time.

```{r}
### Iteratively merge count files
countdata <- 
   foreach(fname = cntfiles, .combine = function(df1,df2){dplyr::full_join(df1, df2, by = "gene")}) %do% {
