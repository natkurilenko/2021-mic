...
Carry out a basic set of DE analyses using DESeq2 and visualize the results

Requires the following:
- ./proc//PRJNA668393-ddsdata.RDS


## Load packages and data object
...
DESeqDataSet is an S4 object. Recall that an S4 object was taught when introducing bioconductor. Note that S4 objects allow users to wrap up multiple elements into a single variables where each element is called a slot.
...
- **Effect size**
  ...
  - Treatment effect within the UV2 cell line ($z_j=1$)
	- $\Delta_{i1} = (\beta_{0i} + \beta_{1i} + \beta_{2i}) - (\beta_{0i} + \beta_{2i}) = \beta_{1i}$

- **Parameter interpretation**
	- $\beta_{1i}$ quantifies the treatment effect
	- Under the additive model, the treatment effect does not depend on the cell line: $\Delta_{0i} = \Delta_{1i} = \beta_{i1}$
...
DESeq2::counts(ddsobj)[102:104,c(3,11)]
```

```{r}
# Size factors for samples 3 and 11

DESeq2::sizeFactors(ddsobj)[c(3,11)]
```

```{r}
# normalized count for samples 3 and 11
DESeq2::counts(ddsobj, normalize = TRUE)[102:104, c(3,11)]
```

```{r}
# normalized manually using size factors for sample 3
DESeq2::counts(ddsobj)[102:104, 3] / DESeq2::sizeFactors(ddsobj)[3]
```

```{r}
# normalized manually using size factors for sample 11
DESeq2::counts(ddsobj)[102:104, 11] / DESeq2::sizeFactors(ddsobj)[11]
```
...
### Extract and interpret coefficient estimates
...
- Parameter estimates for ENSMUSG00000110537.2

    * $\hat{\beta}_{i0} = 5.240018$
    * $\hat{\beta}_{i1} = -1.302921$
    * $\hat{\beta}_{i2} = -0.2218365$


- Estimated mean counts

    * trt == 0 and cline == 0  => $\log2(q_{ij})$= 5.240018	+ (0) + (0)  = 5.240018
    * trt == 1 and cline == 0  => $\log2(q_{ij})$= 5.240018	+ (-1.302921) + (0) = 3.937097
    * trt == 0 and cline == 1  => $\log2(q_{ij})$= 5.240018	+ (0) + (-0.2218365) = 5.018181
    * trt == 1 and cline == 1  => $\log2(q_{ij})$= 5.240018	+ (-1.302921) + (-0.2218365) = 3.715261

- The treatment DE effect under the additive model does not depend on cell line

    * if cline == 0 => $\hat{\Delta}_{i0}$ = 3.937097 - 5.240018  = -1.302921
    * if cline == 1 => $\hat{\Delta}_{i1}$ = 3.715261 - 5.018181 = -1.30292

**As exepected $\hat{\Delta}_{i0} = \hat{\Delta}_{i0} = \hat{\beta}_{i1}$**
...
```{r}
outdir <- "./proc"
outfile <- file.path(outdir, "PRJNA668393-DESeq2-additive.RDS")

saveRDS(ddsDE, file = outfile)
