...
Remove those na values in the ranks.
...
Assign names to the `ranks` vector. 
...
##### Another way to include the gene id version (not recommended).

```{r}
trial <- getBM(attributes =c('entrezgene_id', 'ensembl_gene_id', 'ensembl_gene_id_version'),
...
Here $21.64\%$ is a relatively large percentage. However, this is because we use the mice data with a small sample size. It is not surprising that the statistics will have lots of ties. But most of the tied statistics are non-significant. So it won't be a big problem here.
...
dds.fit <- DESeq(dds)
```
Check the results of DESEQ2 fitting.
```{r}
res_int <- results(dds.fit)
head(res_int)
```

#### Extract the statistcs
...
Please follow the above examples and finish the listed tasks by yourself.

1. Extract the statistics. 

1. Mapping the gene annotations
