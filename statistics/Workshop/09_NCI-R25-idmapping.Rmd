...
### Check the annotation with the gene verison number 

```{r}
readRDS(addfile) %>%
    DESeq2::results(tidy = TRUE) %>%
    tidyr:: drop_na(pvalue) -> res_add
```


```{r}
genemap <- biomaRt::getBM(
    filters = "ensembl_gene_id_version",
    attributes=c("ensembl_gene_id", "ensembl_gene_id_version", "entrezgene_id","mgi_id", "mgi_symbol"),
    values=res_add$row,
    mart=mymart  
)   
```
...
```{r}
genemap %>% 
  dplyr::group_by(ensembl_gene_id_version) %>% 
  dplyr::filter(n()>1) %>% 
  arrange(ensembl_gene_id_version) %>% head(10)
```

### Remove the gene verision number
