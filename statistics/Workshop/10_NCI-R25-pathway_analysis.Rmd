...
Now check the gene ensembl names. Clearly some of the gene ensembl names contain gene version id. For gene annotation, we need to keep the first part of the name and remove the part after the `.`.

```{r}
# Find those non-na indices
head(res_add$row)
head(str_remove(res_add$row, ".[0-9][0-9]{0,2}$"))
res_add %>% dplyr::mutate(ensid = str_remove(row, ".[0-9][0-9]{0,2}$")) -> res_add
```
...
The whole part can be simplified into the following code.
