...
- [Preparatory steps](#prep)
- [Import meta and count data](#import)
- [Create DESeq2 data object](#object)
- [Gene level analysis: additive model](#add)
- [Gene level analysis: multiplicative model](#mult)
- [Gene level analysis: stimate effect size within cell line](#group)
- [Pathway level analysis: prepare annotation](#pathprep)
- [Pathway level analysis: additive model](#pathadd)
- [Pathway level analysis: multiplicative model](#pathmult)
- [Session Information](#session)



## Preparatory steps <a name="prep"></a>
...
## Import meta and count data <a name="import"></a>
...
## Create DESeq2 data object <a name="object"></a>
...
## Gene level analysis: Additive Model <a name="add"></a>
...
```{r}
design(ddsADD) <- formula(~ cline + trt)
```

### Differential Expression analysis

```{r}
ddsADD <- DESeq2::DESeq(ddsADD)
```
...
```{r}
DESeq2::plotCounts(ddsADD, "ENSMUSG00000110537.2", intgroup = "trt", normalized = TRUE)
```

```{r}
DESeq2::plotCounts(ddsADD, "ENSMUSG00000022564.8", intgroup = "trt", normalized = TRUE)
```
...
## Gene level analysis: Multiplicative Model <a name="mult"></a>


### Create a new copy of the DESEq2 data object 

```{r}
ddsMULT <- PRJNA668393ddsData
```

### Specify design (multiplicative model)

```{r}
design(ddsMULT) <- formula(~ cline + trt + cline:trt)
```

### Differential Expression analysis

```{r}
ddsMULT <- DESeq2::DESeq(ddsMULT)
```
...
```{r}
DESeq2::summary(results(ddsMULT), alpha = 0.2)
```
...
## Gene level analysis: Estimate effect size within cell line <a name="group"></a>

### Create a new copy of the DESEq2 data object 

```{r}
ddsGRP <- PRJNA668393ddsData
```


### add group variable combining the cell line and treatment effect
```{r}
ddsGRP$group <- as.factor(paste0(ddsGRP$trt, ddsGRP$cline))
```
...
## Pathway level analysis: Prepare annotation <a name="pathprep"></a>
...
```
## Pathway level analysis: additive model <a name="pathadd"></a>
...
```
## Pathway level analysis: multiplicative model <a name="pathmult"></a>
...
## Session Information <a name="session"></a>
